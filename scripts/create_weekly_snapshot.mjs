...
async function queryActivitySnapshot(client, regionGroup, snapshotDate) {
  const totals = await client.query(
    `select
       ...
       count(*) filter (where a.tracker_scope = 'applications' and coalesce(a.lodgement_date, a.observed_at) >= $2::date)::int as recent_application_count,
       count(*) filter (
       ...
       count(*) filter (
         where coalesce(a.lodgement_date, a.observed_at) >= $2::date
           and (

...
function constraintTypeLabel(value) {
  const labels = {
    ...
    low_tree_canopy_proxy: 'low tree canopy',
    biodiversity_spatial_sample: 'biodiversity',
    policy_withdrawal_friction: 'policy withdrawal friction'
  }
...
function describeRiskStack(rows, count = 3) {
  const labels = []
  const seen = new Set()

  for (const row of rows) {
    const summary = clean(row.constraint_summary)
    if (!summary) continue

    for (const part of summary.split(',')) {
      const rawType = clean(part).replace(/\s*\(.*?\)\s*$/, '')
      if (!rawType || seen.has(rawType)) continue
      seen.add(rawType)
      labels.push(constraintTypeLabel(rawType))
  ...
  return labels.length ? `${listPhrase(labels)} signals` : ''
}
...
function buildHeadline(topA, constrained, shortlist) {
  ...
  const riskStack = describeRiskStack(constrained)

  if (leaders.length && risked.length) {
    return `${leaders.join(', ')} currently screen as the strongest priority precincts, while ${risked.join(', ')} should be downgraded to a high-risk watchlist because ${riskStack || 'multiple risk and friction signals'} are now stacking together.`
  }
  if (leaders.length) {
...
function buildExecutiveSummary(topA, constrained, councilRanking, pipeline, constraintRows, shortlist) {
  ...
  if (constrained.length) {
    ...
    bullets.push(`${names || 'Several high-activity precincts'} are now showing obvious risk stacking and are better treated as a risk-adjusted watchlist instead of immediate acquisition priorities.${riskStack ? ` The currently surfaced stack is ${riskStack}.` : ''}`)
  } else if (constraintRows.length) {
    const leadingSignals = listPhrase(
      constraintRows
        .filter((row) => row.severity === 'high')
        .slice(0, 3)
        .map((row) => constraintTypeLabel(row.constraint_type))
    )
    if (leadingSignals) bullets.push(`High-severity risk coverage remains visible across the broader region, led by ${leadingSignals}.`)
  }
...
async function main() {
  ...
  try {
    ...
    const shortlist = await client.query(`select v.precinct_name, v.council_name, v.opportunity_rating, v.policy_score, v.friction_score, v.timing_score, v.recent_application_count, v.active_pipeline_count, v.constraint_summary, v.recommended_action, v.trigger_summary from public.v_precinct_shortlist v join public.councils c on c.canonical_name = v.council_name where c.region_group = $1 order by case v.opportunity_rating when 'A' then 1 when 'B' then 2 else 3 end, v.friction_score asc nulls last, v.timing_score desc nulls last, v.recent_da_count desc nulls last, v.active_pipeline_count desc nulls last, v.precinct_name limit 10`, [options.regionGroup])
    const constrained = await client.query(`select v.precinct_name, v.council_name, v.opportunity_rating, v.friction_score, v.constraint_summary, v.recent_application_count, v.active_pipeline_count from public.v_precinct_shortlist v join public.councils c on c.canonical_name = v.council_name where c.region_group = $1 and v.constraint_count > 0 order by v.friction_score desc, v.timing_score desc nulls last, v.recent_da_count desc nulls last, v.active_pipeline_count desc nulls last, v.precinct_name limit 10`, [options.regionGroup])
    ...
    const totalSignalsTracked = applicationTypeMix.rows.reduce((sum, row) => sum + Number(row.total || 0), 0)
    ...
    const markdown = [
      ...
      `- Total signals tracked (applications + SSD): \`${formatNumber(totalSignalsTracked)}\``,

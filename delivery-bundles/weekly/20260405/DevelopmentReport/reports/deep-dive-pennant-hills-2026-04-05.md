# Deep Dive: Pennant Hills

## Date

2026-04-05

## Executive Summary

Pennant Hills is best viewed as a medium-conviction precinct where policy and activity are present, but the case still needs more targeted diligence before it can move into top-tier priority.

## Quick Scorecard

- Precinct council context: `Hornsby`
- Current rating: `B`
- Policy score: `0`
- Timing score: `4`
- Friction score: `2`
- Recent applications: `188`
- Recent application window start: `2025-01-01`
- Active planning proposals: `0`
- State significant projects: `0`
- Recommended action: `Investigate`
- Map centroid: `-33.737499, 151.060138`

## Why This Precinct Surfaced

- Trigger summary: 0 active proposals, 188 recent applications (73 DA, 81 CDC), 0 state significant projects. Risks: bushfire_spatial_sample (high)
- Recent development activity is sufficiently strong to justify immediate attention.

## Policy And Planning Context

- Current score uses `0` active planning proposal items.
- Current mapped source rows at active stages: `0`. Distinct rows shown below after de-duplication / evidence cleanup: `0`.
- Rows below show the currently mapped active proposals behind that count. When a row is not precinct-explicit in its wording, it is still shown and labeled so the score can be audited rather than hidden.

No active mapped proposal records currently surfaced for this precinct.

| Historical Stage | Title | Location |
| --- | --- | --- |
| Made | 12 Schofield Parade, Pennant Hills | 12 Schofield Parade Pennant Hills 2120 |
| Made | 7 City View Road Planning Proposal | 7 City View Road Pennant Hills 2120 |
| Withdrawn | 7 City View Road, Pennant Hills | 7 City View Road Pennant Hills 2120 |

Related council policy context is shown below because current precinct-explicit policy rows are thin; these rows are background context, not direct precinct-scoring evidence.
| Stage | Related Council Policy Context | Location |
| --- | --- | --- |
| Pre-Exhibition | Employment Lands Study - Short Term Recommendations 2025 | Hornsby |
| Made | Hornsby LEP 2013 - 62 Manor Road, Hornsby | 62 Manor Road Hornsby 2077 |
| Made | Hornsby LEP 2013 - Design excellence for tall buildings | Hornsby |
| Made | Hornsby LEP 2013 (Amendment No 8) - Rectify a number of anomalies (mapping, heritage) and reclassification at 111X New Line Road, Cherrybrook (0 dwellings) | Whole Of Lga & more |
| Made | Hornsby Townhouse Planning Proposal | Hornsby 2127 & more |
| Made | Hornsby West Precinct | Hornsby |

## Parcel-Level Planning Signals

Current open-data control readings for representative screened sites are shown below so the precinct thesis can be anchored to parcel-level planning settings rather than council-wide policy alone.
| Representative Site | Zoning | Height | FSR | Minimum Lot Size | Jurisdiction | Sources |
| --- | --- | --- | --- | --- | --- | --- |
| 130 VICTORIA ROAD WEST PENNANT HILLS | R2 (Low Density Residential) | 8.5m | - | 500 m² | Hornsby | [Zoning](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/2) / [FSR](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/1) / [Height](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/5) / [Lot size](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/4) |
| 18 BOYD AVENUE WEST PENNANT HILLS | R2 (Low Density Residential) | 8.5m | - | 500 m² | Hornsby | [Zoning](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/2) / [FSR](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/1) / [Height](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/5) / [Lot size](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/4) |
| 2A FAIRBURN AVENUE WEST PENNANT HILLS | R2 (Low Density Residential) | 8.5m | - | 500 m² | Hornsby | [Zoning](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/2) / [FSR](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/1) / [Height](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/5) / [Lot size](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/Planning/EPI_Primary_Planning_Layers/MapServer/4) |

## Development Activity Context

Recent applications below are screening signals with lodgement date on or after `2025-01-01`. The application-type table is shown as mutually exclusive DA / CDC / Modification / Other buckets so it stays consistent with the `Recent applications` count above. State-significant signals stay separate in the quick scorecard.
Current mix note: this precinct is presently **CDC-led** (81 CDC vs 73 DA), so raw activity volume should not be read as pure acquisition-quality momentum.

| Status | Count |
| --- | --- |
| Determined | 82 |
| Approved | 68 |
| Under Assessment | 22 |
| Additional Information Requested | 8 |
| Withdrawn | 7 |
| Deferred Commencement | 1 |

| Application Type | Count |
| --- | --- |
| DA | 73 |
| CDC | 81 |
| Modification | 33 |
| Other | 1 |

| Lodgement | Status | Type | Location |
| --- | --- | --- | --- |
| 2026-04-01 | Approved | Complying Development Certificate Application | 2A FAIRBURN AVENUE WEST PENNANT HILLS 2125 |
| 2026-04-01 | Under Assessment | Development Application | 154 VICTORIA ROAD WEST PENNANT HILLS 2125 |
| 2026-03-31 | Under Assessment | Modification Application | 53-119 BOUNDARY ROAD PENNANT HILLS 2120 |
| 2026-03-31 | Additional Information Requested | Development Application | 107 CARDINAL AVENUE WEST PENNANT HILLS 2125 |
| 2026-03-31 | Additional Information Requested | Development Application | 53-119 BOUNDARY ROAD PENNANT HILLS 2120 |
| 2026-03-31 | Under Assessment | Development Application | 21 SHIELDS LANE PENNANT HILLS 2120 |
| 2026-03-26 | Additional Information Requested | Development Application | 11 CECIL AVENUE PENNANT HILLS 2120 |
| 2026-03-25 | Under Assessment | Complying Development Certificate Application | 48 LAURENCE STREET PENNANT HILLS 2120 |
| 2026-03-24 | Determined | Modification Application | 130 VICTORIA ROAD WEST PENNANT HILLS 2125 |
| 2026-03-23 | Approved | Complying Development Certificate Application | 18 BOYD AVENUE WEST PENNANT HILLS 2125 |
| 2026-03-22 | Under Assessment | Complying Development Certificate Application | 25 VAUGHAN AVENUE PENNANT HILLS 2120 |
| 2026-03-18 | Additional Information Requested | Development Application | 38 GRAYLIND AVENUE WEST PENNANT HILLS 2125 |

## Risk And Friction

Friction is present but not yet disqualifying. The main task is to keep risk visible rather than let strong activity numbers dominate the narrative.

Context proxies can help explain why a precinct is ranked up or down, but they should not be mistaken for parcel-level blockers on their own.

| Constraint Type | Severity | Signal Use | Source | Evidence | Source URL |
| --- | --- | --- | --- | --- | --- |
| bushfire_spatial_sample | high | Transaction-facing screening signal | BushFire Prone Land Spatial Sample | Matched categories: Vegetation Buffer | https://portal.spatial.nsw.gov.au/server/rest/services/Hosted/NSW_BushFire_Prone_Land/FeatureServer/0 |

## Council Context

| Council | 5-year Target | Recent Apps | Active Pipeline | Tree Canopy % | High Heat Vulnerability % |
| --- | --- | --- | --- | --- | --- |
| Hornsby | 5,500 | 1,392 | 1 | 42.5 | 0 |

## What To Do Next

1. Keep `Pennant Hills` in the `B`-rated watchlist and treat `Investigate` as the current workflow state.
2. Use `dashboard/2026-04-05-report.html` to inspect the surrounding precinct cluster before doing any street-level or owner-contact work.
3. Explicitly validate the current risk stack (bushfire_spatial_sample) before promoting this precinct into a transaction-facing shortlist.

## References

- `dashboard/2026-04-05-report.html`
- `reports/weekly-radar-2026-04-05.md`

# Newcastle-Hunter Planning Opportunity Radar

## Week Of 2026-04-04

Companion visual dashboard: `dashboard/newcastle-hunter-report.html`

## Headline

`Mayfield`, `Broadmeadow`, `Newcastle City Centre` currently screen as the strongest priority precincts, while `Cessnock`, `Kurri Kurri` should be downgraded to a high-risk watchlist because flood, bushfire and friction signals are now stacking together.

## Snapshot

- Planning proposals tracked: `424`
- Application signals tracked: `11,468`
- Precinct shortlist items: `19`
- Derived constraints: `32`

Scope note: the four snapshot numbers above are full region totals for the current source layer, not counts limited to the configured precinct watchlist.

## Executive Summary

- `Mayfield`, `Broadmeadow`, `Newcastle City Centre` currently form the first priority tier because active pipeline and recent applications are both visible while friction is still manageable.
- `Lake Macquarie`, `Newcastle`, `Maitland` remain the strongest council-level activity cluster and are the best candidates for further precinct and street-level work.
- `Cessnock`, `Kurri Kurri`, `Medowie` are now showing obvious risk stacking and are better treated as a risk-adjusted watchlist instead of immediate acquisition priorities. The strongest risk signals currently include flood metadata high hits = 11, bushfire spatial high hits = 15.
- There are still 10 active proposal items in the pipeline, alongside 373 items already in made stage, which is enough to support a sustained planning radar rather than a one-off hotspot view.

## Scoring Method

- This is a first-pass weekly watchlist, not a parcel-level feasibility, pricing or acquisition report.
- Rating combines policy score and timing score, then downgrades precincts when friction stacks.
- Precincts with no current active proposals, mapped recent applications or state-significant projects are forced into `Watch` rather than `Investigate`.
- `Recent Apps` means mapped application signals with lodgement date on or after `2025-01-01`.
- Region totals, configured-precinct watchlists and ranked site-screening outputs do not use the same universe. Read each section by its stated scope.

## Top Precinct Hotlist

| Rank | Precinct | Council | Rating | Policy | Timing | Risk | Recent Apps | Pipeline | Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Mayfield | Newcastle | B | 0 | 5 | 0 | 213 | 0 | Investigate |
| 2 | Broadmeadow | Newcastle | B | 0 | 3 | 0 | 28 | 0 | Investigate |
| 3 | Newcastle City Centre | Newcastle | B | 3 | 5 | 2 | 190 | 1 | Investigate |
| 4 | Nelson Bay | Port Stephens | B | 2 | 5 | 2 | 169 | 1 | Investigate |
| 5 | Rutherford | Maitland | B | 0 | 5 | 2 | 142 | 0 | Investigate |
| 6 | Thornton | Maitland | B | 2 | 5 | 2 | 126 | 1 | Investigate |
| 7 | Kotara | Newcastle | B | 0 | 4 | 2 | 94 | 0 | Investigate |
| 8 | Charlestown | Lake Macquarie | B | 1 | 5 | 3 | 205 | 0 | Investigate |
| 9 | Raymond Terrace | Port Stephens | B | 0 | 5 | 3 | 101 | 0 | Investigate |
| 10 | Medowie | Port Stephens | C | 3 | 5 | 4 | 235 | 1 | Watch |

## Highest Friction Precincts

| Precinct | Council | Risk | Recent Apps | Pipeline | Constraint Summary |
| --- | --- | --- | --- | --- | --- |
| Cessnock | Cessnock | 5 | 215 | 1 | bushfire_spatial_sample (high), flood_metadata_signal (high), policy_withdrawal_friction (medium) |
| Kurri Kurri | Cessnock | 5 | 90 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high), biodiversity_spatial_sample (medium) |
| Medowie | Port Stephens | 4 | 235 | 1 | bushfire_spatial_sample (high), biodiversity_spatial_sample (medium), flood_metadata_signal (medium) |
| East Maitland | Maitland | 4 | 199 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Belmont | Lake Macquarie | 4 | 176 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Warners Bay | Lake Macquarie | 4 | 143 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Maitland | Maitland | 4 | 91 | 1 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Branxton | Cessnock | 4 | 63 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Glendale | Lake Macquarie | 4 | 57 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |
| Williamtown | Port Stephens | 4 | 21 | 0 | bushfire_spatial_sample (high), flood_metadata_signal (high) |

## Council Scoreboard

| Council | Target | Recent Apps | Active Pipeline |
| --- | --- | --- | --- |
| Lake Macquarie | - | 3,555 | 1 |
| Newcastle | - | 2,419 | 1 |
| Maitland | - | 2,353 | 2 |
| Cessnock | - | 1,774 | 1 |
| Port Stephens | - | 1,238 | 5 |

## Recent Application Mix

Current screen window: `2025-01-01`. DA, CDC, SSD and Modification are shown separately because they do not carry the same deal-screening meaning.

| Application Type | Count |
| --- | --- |
| DA | 6,256 |
| CDC | 3,629 |
| SSD | 129 |
| Modification | 1,440 |
| Other | 14 |

## Policy Pipeline

| Stage | Proposal Count | Council Count |
| --- | --- | --- |
| Under Assessment | 3 | 1 |
| Pre-Exhibition | 2 | 2 |
| On Exhibition | 3 | 3 |
| Finalisation | 2 | 2 |
| Made | 373 | 5 |
| Withdrawn | 41 | 5 |

## Proposal Watchlist

| Stage | Council | Title | Location |
| --- | --- | --- | --- |
| Under Assessment | Port Stephens | Medowie Precinct J | Port Stephens |
| Under Assessment | Port Stephens | Nelson Bay Bowling Club Planning Proposal | 61A Dowling Street Nelson Bay 2315 & more |
| Under Assessment | Port Stephens | The Sandpiper (Precinct1A) | 16 George Street Fullerton Cove 2318 & more |
| Pre-Exhibition | Newcastle | Eden Estates, Wallsend | Newcastle |
| Pre-Exhibition | Port Stephens | Reclassification and Rezoning of Surplus Council Sites | Port Stephens |
| On Exhibition | Lake Macquarie | Garden Suburb Project | Lake Macquarie |
| On Exhibition | Maitland | Reclassification of Council Owned Land from Community to Operational | 17 Avard Close Thornton 2322 |
| On Exhibition | Port Stephens | Tomago Industrial Site Planning Proposal | Port Stephens |
| Finalisation | Maitland | AMENDMENT TO THE MAITLAND LEP 2011 Stage 4 of the Aberglasslyn URA, Kezia Rd, Oakhampton Various Lot | Maitland |
| Finalisation | Cessnock | RECLASSIFICATION OF PART LOT 312 DP 566724 OLD MAITLAND ROAD CESSNOCK | Lot 312 Dp566724 |

## Derived Risk Mix

| Constraint Type | Severity | Count |
| --- | --- | --- |
| biodiversity_spatial_sample | medium | 2 |
| bushfire_spatial_sample | high | 15 |
| flood_metadata_signal | high | 11 |
| flood_metadata_signal | medium | 2 |
| policy_withdrawal_friction | medium | 2 |

## Suggested Next Actions

1. Prioritise `A`-rated precincts with low friction for deeper street-level or owner-contact research.
2. Keep high-activity but high-friction precincts in a separate risk-adjusted watchlist rather than the main acquisition shortlist.
3. Use `dashboard/newcastle-hunter-report.html` to visually inspect clusters before writing the next deep-dive memo.
